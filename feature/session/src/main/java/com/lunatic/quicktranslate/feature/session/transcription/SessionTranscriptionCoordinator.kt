...
class SessionTranscriptionCoordinator(
    ...
    private fun prepareMediaPath(mediaUri: String): PreparedMedia {
        ...
        if (scheme.equals("file", ignoreCase = true)) {
            return PreparedMedia(path = uri.path.orEmpty())
        }
        return PreparedMedia(path = mediaUri)
    }
    ...
    private fun convertFloatPcmTo16Bit(floatBytes: ByteArray): ByteArray {
        ...
        while (input.remaining() >= 4) {
            ...
            val pcm = (value * Short.MAX_VALUE).toInt().coerceIn(Short.MIN_VALUE.toInt(), Short.MAX_VALUE.toInt())
